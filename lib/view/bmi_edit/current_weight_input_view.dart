...
import 'target_weight_input_view.dart';
...
class CurrentWeightInputView extends StatefulWidget {
  ...
  @override
  State<CurrentWeightInputView> createState() => _CurrentWeightInputViewState();
}

class _CurrentWeightInputViewState extends State<CurrentWeightInputView> {
  ...
  void _initializeData() {
    ...
    if (userProvider.user != null && userProvider.user!.weight > 0) {
      _selectedWeight = userProvider.user!.weight;
    }
  }
  ...
  String _getDisplayWeight() {
    if (_isMetric) {
      ...
      // Convert kg to lb
      double weightInLb = _selectedWeight * 2.20462;
  ...
  double _calculateBMI() {
    ...
    return _selectedWeight / (heightInM * heightInM);
  }

  String _getBMIAdvice() {
    double bmi = _calculateBMI();
    if (bmi < 18.5) return "Bạn cần tăng cân để có sức khỏe tốt hơn";
    if (bmi < 25) return "Bạn có một chỉ số tuyệt vời! Hãy duy trì nhé!";
